...
// generic shellsort, accepts a gap distribution
static void fr_shellsort_double_i_(const double * a,
                                   MY_SIZE_T indx[],
                                   const MY_SIZE_T n,
                                   const MY_SIZE_T * the_gaps,
                                   const int N_GAPS);
// quicksort with insertion sort when N <= crit_size
 ...
 * filled with 0..n-1.
 *
 * Insertion sort can be more efficient than quicksort for short vectors,
 * and my benchmarking shows that, at least on my laptop, a crit_size of
 * 21 so all vectors length 20 or less are insertion-sorted is better than
 * the default crit_size = 2, and also better than crit_size = 11.
 */
 ...
 * suggested at the above Wikipedia page.  For randomly-generated
 * data, Sedgwick clearly performs best on longer (n = 10000)
 * vectors, while Ciura and Tokuda are slightly better for shorter
 * vectors.
...
#  define N_GAPS_SEDGWICK2 20
#  define N_GAPS_TOKUDA    35
#  define N_GAPS_TOKUDA2   33
#  define N_GAPS_CIURA     35
#  define N_GAPS_CIURA2    33
#else
   /* Here, gaps up to just under 2^31 - 1, MAXINT */
#  define N_GAPS_SEDGWICK  17
#  define N_GAPS_SEDGWICK2 16
#  define N_GAPS_TOKUDA    27
#  define N_GAPS_TOKUDA2   25
#  define N_GAPS_CIURA     26
#  define N_GAPS_CIURA2    24
#endif
static const MY_SIZE_T ciura2_gaps [N_GAPS_CIURA2] = {
#ifdef LONG_VECTOR_SUPPORT
...
static const MY_SIZE_T sedgwick2_gaps [N_GAPS_SEDGWICK2] = {
#ifdef LONG_VECTOR_SUPPORT
...
static const MY_SIZE_T tokuda2_gaps [N_GAPS_TOKUDA2] = {
#ifdef LONG_VECTOR_SUPPORT
...
SEXP fastrank_(SEXP s_x, SEXP s_tm, SEXP s_sort) {
    ...
    if (sort_method < 1 || sort_method > 10)
        error("'sort' must be between 1 and 10");
    enum { R_ORDERVECTOR = 1,
           ...
           FR_QUICKSORT_21,
           FR_SHELLSORT_CIURA2,
           FR_SHELLSORT_SEDGWICK2,
           FR_SHELLSORT_TOKUDA2 };
    ...
    case R_ORDERVECTOR:
        ...
        break;
    case FR_QUICKSORT:
        error("sort.method 2 (Quicksort crit 2) already off the island");
        /* fr_quicksort_double_i_(REAL(s_x), indx, n, 2); */
        break;
    case FR_SHELLSORT_CIURA:
        fr_shellsort_double_i_(REAL(s_x), indx, n, &ciura_gaps[0], N_GAPS_CIURA);
        break;
    case FR_SHELLSORT_SEDGWICK:
        fr_shellsort_double_i_(REAL(s_x), indx, n, &sedgwick_gaps[0], N_GAPS_SEDGWICK);
        break;
    case FR_SHELLSORT_TOKUDA:
        fr_shellsort_double_i_(REAL(s_x), indx, n, &tokuda_gaps[0], N_GAPS_TOKUDA);
        break;
    case FR_QUICKSORT_11:
        fr_quicksort_double_i_(REAL(s_x), indx, n, 11);
        break;
    case FR_QUICKSORT_21:
        fr_quicksort_double_i_(REAL(s_x), indx, n, 21);
        break;
    case FR_SHELLSORT_CIURA2:
        fr_shellsort_double_i_(REAL(s_x), indx, n, &ciura2_gaps[0], N_GAPS_CIURA2);
        break;
    case FR_SHELLSORT_SEDGWICK2:
        fr_shellsort_double_i_(REAL(s_x), indx, n, &sedgwick2_gaps[0], N_GAPS_SEDGWICK2);
        break;
    case FR_SHELLSORT_TOKUDA2:
        fr_shellsort_double_i_(REAL(s_x), indx, n, &tokuda2_gaps[0], N_GAPS_TOKUDA2);
        break;
    default:
...
SEXP fastrank_num_avg_(SEXP s_x) {
    ...
    for (MY_SIZE_T i = 0; i < n; ++i) indx[i] = i;
    /* quicksort with insertion sort under length 21 */
    fr_quicksort_double_i_(x, indx, n, 21);
    if (DEBUG) {
        Rprintf(" indx:   ");
        for (int i = 0; i < n; ++i) Rprintf("%d    ", indx[i]);
        Rprintf("\n");
